...
pub fn run_to_vblank(nes: &mut Nes) {
    ...
    let mut until_blank = ppu_catchup(nes, nes.cpu.cycles*3);
    let mut ppu_depend = false;

    while (until_blank > 0) {
        ppu_depend = step_cpu(nes);
        if ppu_depend {
            until_blank = ppu_catchup(nes, nes.cpu.cycles*3);
            // println!("Where we at {}", until_blank);
        }
    }
...
fn ppu_catchup(nes: &mut Nes, target: u64) -> u32 {
    while nes.ppu.cycles != target {
        
        // let nametable_addr = (0x2000 + 0x0400 * (nes.ppu.nametable_select as u16));
    ...
    let balls = ((nes.ppu.scanline as u32)*256) + nes.ppu.pixel as u32;
    if balls <= 27508*3 {
        println!("Balls number: {}", balls);
        thread::sleep(time::Duration::from_millis(10));
        (27508*3 - balls)
    } else {
        (29680*3 - balls) + 27508*3
    }
...
fn step_cpu(nes: &mut Nes) -> bool {
    ...
    let expected =  pc_list[nes.cpu.counter as usize];

    if expected != nes.cpu.pc {
        panic!("AAHHH, {:02X?} instead of {:02X?}", nes.cpu.pc, expected);
    }
...
fn exec_instruction(opcode: u8, instr_addr: u16, instr_val: u8, nes: &mut Nes) {
    match opcode {
        ...
        0xED | 0xFD | 0xF9 | 0xE9 | 0xE1 | 0xF1 | 0xE5 | 0xF5 | 0xEB => {
            let (result, borrow) = nes.cpu.a.borrowing_sub(instr_val, !nes.cpu.p_c);
            nes.cpu.p_v = was_signed_overflow(nes.cpu.a, (-(instr_val as i8) as u8), result);
            nes.cpu.a = result;
            nes.cpu.p_n = is_neg(nes.cpu.a);
            nes.cpu.p_z = nes.cpu.a == 0;
            nes.cpu.p_c = !borrow;
        },
        // DEC
        0xCE | 0xDE | 0xC6 | 0xD6 => {
