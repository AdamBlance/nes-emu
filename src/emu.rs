...
pub fn run_to_vblank(nes: &mut Nes) {
    ...
    // Instead of doing this CPU ppu catchup thing, could also just run CPU for a tick, run PPU for a tick
...
fn step_ppu(nes: &mut Nes) {
    ...
    // println!("scanline: {}, pixel: {}", nes.ppu.scanline, nes.ppu.pixel);
    ...
    if nes.ppu.pixel < 256 && nes.ppu.scanline < 240 {
        ...
        let nametable_addr = (0x0400 * (nes.ppu.nametable_select as u16));


        let index = (nametable_addr + (nes.ppu.pixel / 8)) as usize;
        

        let tile_idx = nes.ppu.vram[index] as usize;
        
        let lsb = nes.cart.chr_rom[tile_idx + if nes.ppu.background_tile_select {0x1000} else {0x0000}] & 0x01;
        let msb = nes.cart.chr_rom[tile_idx+8] & 0x01;
        ...
        let frame_idx = (nes.ppu.scanline*256 + nes.ppu.pixel) as usize;
        ...
        // println!("{}-{}-{}", nes.frame[frame_idx], nes.frame[frame_idx+1], nes.frame[frame_idx+2]);
    }
...
fn step_cpu(nes: &mut Nes) {
    ...
    // if let 0x2000..=0x2007 = instr_addr {
    //     return false;
    // }
    ...
    // let correct = log::LOG[nes.cpu.counter as usize];

    println!("{}", log_line);

    // if log_line != correct {
        // println!("Not what was expected!");
        // println!("Got:\n{log_line}\nInstead of:\n{correct}\n");
        // panic!();
    // }
